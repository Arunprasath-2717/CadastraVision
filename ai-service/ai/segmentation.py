...
class CadastralSegmentationPipeline:
    ...
    def inference_tile(self, image_np: np.ndarray) -> np.ndarray:
        """
        Runs model forward pass to produce a 2D binary raster mask.

        Args:
            image_np: Sub-tile numpy array of shape (H, W, C).

        Returns:
            A binary mask numpy array of shape (H, W) where 255 is foreground, 0 is background.
        """
        ...
        if self.backend == "samgeo":
            ...
            if torch.is_tensor(masks_tensor):
                # Combine all masks by taking the logical OR
                merged = (masks_tensor.sum(dim=0) > 0).cpu().numpy().astype(np.uint8) * 255
            else:
                merged = (np.sum(masks_tensor, axis=0) > 0).astype(np.uint8) * 255
            return merged
        ...
        with rasterio.open(geotiff_path) as src:
            ...
            for y in range(0, height, stride):
                ...
                for x in range(0, width, stride):
                    ...
                    tile_mask = self.inference_tile(tile_rgb)

                    # Local-to-CRS transform for this window
                    window_transform = src.window_transform(window)

                    # Extract polygons in native CRS space
                    crs_polygons = self.mask_to_polygons(tile_mask, window_transform, min_area=30.0)
                    ...
                    for crs_poly in crs_polygons:
                        ...
                        min_x = max(0, min_x)
                        min_y = max(0, min_y)
                        max_x = min(tile_mask.shape[1], max_x)
                        max_y = min(tile_mask.shape[0], max_y)

                        if (max_x - min_x) <= 0 or (max_y - min_y) <= 0:
                            continue

                        mask_crop = tile_mask[min_y:max_y, min_x:max_x]
